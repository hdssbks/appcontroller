...
**Note:** go-get or vendor this package as `appcontroller`.
...
The app controller uses [client-go library](https://github.com/kubernetes/client-go/tree/master/tools/cache) extensively.
The details of interaction points of the app controller with various mechanisms from this library are
explained [here](docs/controller-client-go.md).

## Fetch appcontroller and its dependencies

Like the rest of Kubernetes, appcontroller has used
...
```sh
go get -d appcontroller
cd $GOPATH/src/appcontroller
godep restore
```
...
```sh
git clone https://github.com/kubernetes/appcontroller.git
cd appcontroller
```
...
`kubernetes/staging/src/appcontroller` and its dependencies
...
**Prerequisite**: Since the appcontroller uses `apps/v1` deployments, the Kubernetes cluster version should be greater than 1.9.

```sh
# assumes you have a working kubeconfig, not required if operating in-cluster
go build -o appcontroller .
./appcontroller -kubeconfig=$HOME/.kube/config
...
```sh
kubectl delete crd Apps.appcontroller.k8s.io
```
...
`appcontroller` is synced from
https://github.com/kubernetes/kubernetes/blob/master/staging/src/appcontroller.
